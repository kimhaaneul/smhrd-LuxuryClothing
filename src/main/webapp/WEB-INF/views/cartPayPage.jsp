...
<body>
   ...
   <!-- ***** Preloader Start ***** -->
    ...
    <table border="1" align="center" class="fuck">
        ...
        <c:set var="firstProdName" value="${prodList[0].prod_name}"></c:set>
    	...
    	<c:forEach var="prod" items="${prodList}">
	        ...
	        <c:set var="totalAmount" value="${totalAmount + prod.prod_price}"/>
        </c:forEach>
        	<c:set var="count" value="${count-1}"></c:set>
        <c:if test="${count}>0">
	        <c:set var="prodsName" value="${firstProdName}${' 외 '}${count}${'개'}"/>
        </c:if>
		<c:if test="${count==0}">
	        <c:set var="prodsName" value="${firstProdName}"/>
		</c:if>        
    </table>
    ...
    <div class="rufwp" style="border: 1px solid black; padding: 10px; position=relative top=200px">
        <!-- 상품의 총 금액을 보여줄 부분 -->
        <span style="float: right;">총 결제금액 : ${totalAmount}원</span>
    ...
    <script>
        ...
        function requestPay() {
          ...
          IMP.request_pay({
            pg: "kakaopay",
            pay_method: "card",
            merchant_uid: "test_werwerwersdf${prod.prod_id}",
            name: "${prodsName}",
            amount: ${totalAmount},
            popup: true,
             ...
             // callback
              if (rsp.success) {
                 ...
                 console.log(deli_name);
                   ...
                   $.ajax({
                       url: 'cartPaySuccess.do', // 서버로 전송할 URL
                       type: 'POST', // 전송 방식
